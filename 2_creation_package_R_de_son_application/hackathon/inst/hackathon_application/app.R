ui <- fluidPage(
  includeCSS(system.file("www", "styles.css", package = "hackathon")),
...
server <- function(input, output) {
  ...
  data <- reactive({
    if (input$dataset == "newcomb") {
      ...
      x <- list(
        values = unlist(read.csv(system.file("extdata", "newcomb.csv", package = "hackathon"))[2]),
        main = "Newcomb")

    } else if (input$dataset == "forbes") {
      message("Loading Forbes dataset")
      forbes <- read.csv(system.file("extdata", "forbes.csv", package = "hackathon")) ## Load the forbes data table
      x <- list(
      ...
      x <- list(
        values = unlist(read.delim(system.file("extdata", "euros.tsv", package = "hackathon"))[1]),
        main = "Euros")
